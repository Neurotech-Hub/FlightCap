...
| TOF_EN    | `P0.09`        | TPS63900 enable — **HIGH** powers VL53L0X rail (`flightcap-prod` only)            |
...
- **External flash**: MX25R8035F (8 Mb SPI NOR) on `&spi1` (SPIM1) — instance 0 is shared with TWIM0, so I2C uses `&i2c0` and SPI uses `&spi1`. DTS node present; used in prod phase only.
...
- **Driver**: [`applications/flightcap-axy/src/lis2dh12_zephyr.c`](applications/flightcap-axy/src/lis2dh12_zephyr.c) over I2C `0x19`; register helpers in [`lib/lis2dh12/`](lib/lis2dh12/).
- **Build**: `west build -b FlightCap_nRF52833 applications/flightcap-axy`

### 3. `applications/flightcap-ble-test`
...
### 4. `applications/flightcap-prod`

- **Purpose**: Production firmware — 1 Hz BLE telemetry (motion + distance), magnet forced-sleep, MCUmgr DFU.
...
| Path | Purpose |
| --- | --- |
| [`lib/vbatt/`](lib/vbatt/) | SAADC VDD read (`vbatt_init`, `vbatt_read_mv`) via `zephyr,user` → `&adc` channel 0 |
| [`lib/lis2dh12/`](lib/lis2dh12/) | Minimal LIS2DH12 register defs + temperature conversion |
...
1. RTT shows `LIS2DH12 ready WHO_AM_I=0x33 I2C addr=0x19`.
